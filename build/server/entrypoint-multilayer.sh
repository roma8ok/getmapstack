...
# tileparts: begin
# This image ships its vector tiles in parts, one registry layer each, because the
# archive is too large for one. tileparts serves the parts joined on loopback and Martin
# reads the basemap from it over HTTP. Martin resolves its sources at startup, so the
# port has to answer before Martin starts; tileparts checks the parts before it listens,
# so an answering port also means the parts are whole. A tileparts that dies at once
# ends the wait early instead of after the full ten seconds.
tileparts &
TILEPARTS_PID=$!
PIDS+=($TILEPARTS_PID)
for _ in $(seq 1 100); do
  (exec 3<>/dev/tcp/127.0.0.1/3001) 2>/dev/null && break
  kill -0 "$TILEPARTS_PID" 2>/dev/null || break
  sleep 0.1
done
if ! (exec 3<>/dev/tcp/127.0.0.1/3001) 2>/dev/null; then
  echo "entrypoint: tileparts is not serving the vector tiles" >&2
  shutdown
  exit 1
fi
# tileparts: end
